		...
		onTheMasterPage : {
			...
			actions : {
				...
				iSortTheListOnName : function () {
					return this.iChooseASorter("sortButton", "Sort By <BindingPeriodValidityEndDate>");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "A_RequestForQuotation",
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("RequestForQuotation") === oObject.RequestForQuotation;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].RequestForQuotation;
									} else {
										sCurrentId = aItemsNotInTheList[0].RequestForQuotation;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("A_RequestForQuotation", {
										RequestForQuotation : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("RequestForQuotation"),
						title: oBindingContext.getProperty("BindingPeriodValidityEndDate")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<A_RequestForQuotation> (0)"}),
						success: function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("BindingPeriodValidityEndDate");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all A_RequestForQuotation to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<A_RequestForQuotation> (" + iExpectedLength + ")"}),
								success : function () {
